...
const CATEGORIES = [
  { value: 'college_team', label: 'College Team Form', icon: <Users className="h-4 w-4" /> },
  { value: 'self_evaluate', label: 'Self Evaluation', icon: <UserCheck className="h-4 w-4" /> },
  { value: 'students', label: 'Students', icon: <User className="h-4 w-4" /> },
  { value: 'immediate_supervisor', label: 'Immediate Supervisor', icon: <UserCheck className="h-4 w-4" /> },
];
...
const CreateEvaluation = () => {
  ...
  // Add your courses and departments selectors here
  const courses = []; // Replace with your courses from Redux
  const departments = []; // Replace with your departments from Redux
  ...
  const validateForm = () => {
    ...
    formData.criteria.forEach((criteria, index) => {
      ...
      if (criteria.weight <= 0 || criteria.weight > 100) {
        newErrors[`criteria-${index}-weight`] = 'Weight must be between 1 and 100';
      }
    });
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="space-y-1">
                      ...
                      <Input
                        ...
                        className={errors[`criteria-${index}-category`] ? 'border-red-500' : ''}
                      />
                      {errors[`criteria-${index}-category`] && (
                        <p className="text-sm text-red-500">{errors[`criteria-${index}-category`]}</p>
                      )}
                    </div>
                    ...
                    <div className="space-y-1 md:col-span-2">
                      ...
                      <Input
                        ...
                        className={errors[`criteria-${index}-description`] ? 'border-red-500' : ''}
                      />
                      {errors[`criteria-${index}-description`] && (
                        <p className="text-sm text-red-500">{errors[`criteria-${index}-description`]}</p>
                      )}
                    </div>
                    ...
                    <div className="space-y-1">
                      ...
                      <Input
                        ...
                        className={errors[`criteria-${index}-weight`] ? 'border-red-500' : ''}
                      />
                      {errors[`criteria-${index}-weight`] && (
                        <p className="text-sm text-red-500">{errors[`criteria-${index}-weight`]}</p>
                      )}
                    </div>
                  </div>
                </div>
